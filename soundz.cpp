...
#include "BASS/api.h"
#include "soundz.h"
#include "E:/!!paste-uri/Hit Sense/trouble_maker/bass/string_obfuscation.h"
...
void playback_loop()
{
	...
	const auto desired_channel = vars.misc.radiochannel;
	static auto current_channel = 0;

	if (vars.misc.radiochannel == 0)
	{
		current_channel = 0;
		BASS_Stop();
		BASS_STOP_STREAM();
	...
	else if (once && vars.misc.radiochannel > 0)
	{
		...
		BASS_SET_VOLUME(BASS::stream_handle, radio_muted ? 0 : vars.misc.radiovolume / 100);
		BASS_PLAY_STREAM();
	}
