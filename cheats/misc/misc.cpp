...
bool misc::double_tap(CUserCmd* m_pcmd)
{
	...
	if (vars.ragebot.double_tap_shift_value == 0)
	{
		vars.ragebot.double_tap_shift_value = 8;
	}
	...
	if (recharging_double_tap)
	{
		...
		auto recharge_time = csgo.globals.weapon->can_double_tap() ? TIME_TO_TICKS(0.5f) : TIME_TO_TICKS(0.75f);
		...
		if (!aim::get().should_stop && fabs(csgo.globals.fixed_tickbase - last_double_tap) > recharge_time)
		{
			last_double_tap = 0;
			...
			last_double_tap = csgo.globals.fixed_tickbase;
	}
	...
	auto tickbase_shift = min(vars.ragebot.double_tap_shift_value, max_tickbase_shift);

	if (!csgo.globals.weapon->is_grenade() && csgo.globals.weapon->m_iItemDefinitionIndex() != WEAPON_TASER && csgo.globals.weapon->m_iItemDefinitionIndex() != WEAPON_REVOLVER && csgo.send_packet && (((m_pcmd->m_buttons & IN_ATTACK || !m_pcmd->m_buttons & IN_ATTACK2) && !csgo.globals.weapon->is_knife())))
	{
		...
		csgo.globals.tickbase_shift = tickbase_shift;
