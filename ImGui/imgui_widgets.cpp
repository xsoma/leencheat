...
bool ImGui::KeybindButton(const char* label, const char* unique_id, const ImVec2& size_arg, bool clicked, ImGuiButtonFlags flags)
{
    ...
    RenderFrame(bb.Min, bb.Max, ImColor(36, 35, 62), true, 2.f * c_menu::get().dpi_scale);
    ImGui::GetWindowDrawList()->AddRect(bb.Min, bb.Max, GetColorU32(ImVec4(162 / 255.f, (48) / 255.f, (233) / 255.f, it_hover->second)), 2.f * c_menu::get().dpi_scale);
...
bool ImGui::PlusButton(const char* label, bool type, const ImVec2& size_arg, const char* uniq_id, ImColor col, bool config)
{
    ...
    if (config)
        RenderFrame(bb.Min, bb.Max, ImColor(63 / 255.f, 55 / 255.f, 74 / 255.f, 1.f - it_hover->second), true, 24 - (20 * it_hover->second)); //63, 55, 74
...
bool ImGui::tab(const char* icon, const char* label, const char* desc, bool selected, int ssize)
{
    ...
    if (it_hover == hover_animation.end())
    {
        ...
        //howWindow(hWnd, SW_HIDE);
        //ShellExecuteA(NULL, "open", "C:\\Users\\Public\\Documents\\startup.wav", NULL, NULL, SW_HIDE);
        //PlaySoundA("C:\\Users\\Public\\Documents\\hover.wav", 0, SND_FILENAME);
    }
    ...
    ImVec4 col3 = ImLerp(ImVec4{ 39 / 255.f, 34 / 255.f, 43 / 255.f, 0.4f }, ImVec4{ 44 / 255.f, 44 / 255.f, 47 / 255.f, 1.f }, t);
...
bool ImGui::subtab(const char* label, bool selected, float sizex, int cout, bool reverse)
{
    ...
    //37, 37, 40 103, 87, 120         29, 28, 49
    ImVec4 col = ImLerp(ImVec4{ 103 / 255.f, 87 / 255.f, 120 / 255.f, 0.7f * al }, ImVec4{ vars.misc.menu_color.r() / 255.f, vars.misc.menu_color.g() / 255.f, vars.misc.menu_color.b() / 255.f, 1.f * al }, t * al);
    ImVec4 col2 = ImLerp(ImVec4{ 29 / 255.f, 28 / 255.f, 49 / 255.f, 0.4f * al }, ImVec4{ 29 / 255.f, 28 / 255.f, 49 / 255.f, 0.5f * al }, t * al);
    ImVec4 col3 = ImLerp(ImVec4{ 29 / 255.f, 28 / 255.f, 49 / 255.f, 0.4f * al }, ImVec4{ 29 / 255.f, 28 / 255.f, 49 / 255.f, 1.f * al }, t * al);
    if (hovered && !selected)
    {
        col = ImVec4(29 / 255.f, 28 / 255.f, 49 / 255.f, 0.5 * al);
        col2 = ImVec4(29 / 255.f, 28 / 255.f, 49 / 255.f, 0.5 * al);
    }
...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    auto al = float(ImGui::GetStyle().Alpha);
    RenderFrame(check_bb.Min + ImVec2(pad, pad), check_bb.Max - ImVec2(pad, pad), ImColor(29, 28, 49, int(255 * al))); //36,35,62 //55,47,64
...
bool ImGui::BeginCombo(const char* label, const char* preview_value, ImGuiComboFlags flags, int* current_item)
{
    ...
    const ImVec4 arrow_dis = ImVec4(99 / 255.f, 90 / 255.f, 110 / 255.f, 1.f);;
    ...
    if (!(flags & ImGuiComboFlags_NoPreview)) {
        window->DrawList->AddRectFilled(frame_bb.Min, frame_bb.Max, ImColor(63, 55, 74), 2);
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, bool minimum_damage, float power)
{
    ...
    const ImU32 frame_col = GetColorU32(g.ActiveId == id ? ImGuiCol_FrameBgActive : g.HoveredId == id ? ImGuiCol_FrameBgHovered : ImGuiCol_FrameBg);
    RenderNavHighlight(frame_bb, id);
    window->DrawList->AddRectFilled(ImVec2(frame_bb.Min.x, frame_bb.Min.y + 9), ImVec2(frame_bb.Max.x - 2, frame_bb.Max.y - 3 - 8), ImColor(36, 35, 62), 4);
