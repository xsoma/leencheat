...
void draw_keybind(const char* label, key_bind* key_bind, const char* unique_id, bool with_color = false)
{
	...
	ImGui::PushStyleVar(ImGuiStyleVar_::ImGuiStyleVar_PopupRounding, 4); //33 / 255.f, 33 / 255.f, 33 / 255.f,
	ImGui::PushStyleColor(ImGuiCol_PopupBg, ImVec4(29 / 255.f, 28 / 255.f, 49 / 255.f, 1 * 0.85f)); ///////////////////////////fweaf v38, 34, 43 29, 28, 49
...
void lua_edit(std::string window_name)
{
	...
	auto get_dir = [&]() -> void
	{
		...
		if (SUCCEEDED(SHGetFolderPath(NULL, CSIDL_PERSONAL, NULL, NULL, path)))
			file_path = std::string(path) + crypt_str("\\leencheat\\lua\\");
...
void c_menu::keys()
{
	...
	Style->Colors[ImGuiCol_WindowBg] = ImColor(40, 39, 68, 215);//zochem? ia zhe ubral bg?
	...
	ImGui::Begin("KEYS", nullptr, ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoTitleBar | ImGuiWindowFlags_NoBackground | ImGuiWindowFlags_NoCollapse | ImGuiWindowFlags_NoScrollbar | ImGuiWindowFlags_::ImGuiWindowFlags_NoBringToFrontOnFocus | ImGuiWindowFlags_::ImGuiWindowFlags_NoBackground | ImGuiWindowFlags_::ImGuiWindowFlags_NoNavFocus | ImGuiWindowFlags_::ImGuiWindowFlags_NoNav);
	{
		...
		d->AddRectFilled(p, p + ImVec2(200, 21 + 18 * pressed_binds), ImColor(39, 39, 39, int(0 * main_alpha)));
		auto main_colf = ImColor(39, 39, 39, int(0 * main_alpha));
		...
		auto main_colf2 = ImColor(39, 39, 39, int(0 * min(main_alpha * 2, 1.f)));
		...
		if (m_engine()->IsConnected()) {
			add_key("Damage Override", key_binds::get().get_key_bind_state(_MIN_DAMAGE), vars.ragebot.damage_override_key, 200, min(main_alpha * 2, 1.f), rage, true);
			add_key("Legit Key", key_binds::get().get_key_bind_state(_LEGITBOT), vars.legitbot.key, 200, min(main_alpha * 2, 1.f), legit);
			add_key("Auto Fire", key_binds::get().get_key_bind_state(_AUTOFIRE), vars.legitbot.autofire_key, 200, min(main_alpha * 2, 1.f), legit);
			add_key("Doubletap", vars.ragebot.double_tap && misc::get().double_tap_key || misc::get().double_tap_checkc || key_binds::get().get_key_bind_state(_DOUBLETAP), vars.ragebot.double_tap_key, 200, min(main_alpha * 2, 1.f), rage, false);
			...
			add_key("Force Baim", key_binds::get().get_key_bind_state(_BODY_AIM), vars.ragebot.body_aim_key, 200, min(main_alpha * 2, 1.f), rage);
			...
			add_key("Thirdperson", key_binds::get().get_key_bind_state(_THIRDPERSON), vars.misc.thirdperson_toggle, 200, min(main_alpha * 2, 1.f), vis);
			add_key("Anti Aim Inverter", key_binds::get().get_key_bind_state(_DESYNC_FLIP), vars.antiaim.flip_desync, 200, min(main_alpha * 2, 1.f), aa);
...
void c_menu::spectators()
{
	...
	Style->Colors[ImGuiCol_ResizeGripHovered] = ImColor(42, 40, 43, 0);
	Style->Colors[ImGuiCol_ResizeGripActive] = ImColor(42, 40, 43, 0);
	Style->Colors[ImGuiCol_Border] = ImColor(38, 34, 43, 215);
	Style->Colors[ImGuiCol_Button] = ImColor(29, 125, 229, 5);
	Style->Colors[ImGuiCol_ButtonHovered] = ImColor(118, 33, 217, 5);
	Style->Colors[ImGuiCol_ButtonActive] = ImColor(118, 33, 217, 5);
	Style->Colors[ImGuiCol_PopupBg] = ImColor(18, 17, 18, 255);
...
//		return;
	...
	ImGui::Begin("SPECTATORS", nullptr, ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoTitleBar | ImGuiWindowFlags_NoBackground | ImGuiWindowFlags_NoCollapse | ImGuiWindowFlags_NoScrollbar | ImGuiWindowFlags_::ImGuiWindowFlags_NoBringToFrontOnFocus | ImGuiWindowFlags_::ImGuiWindowFlags_NoBackground | ImGuiWindowFlags_::ImGuiWindowFlags_NoNavFocus | ImGuiWindowFlags_::ImGuiWindowFlags_NoNav);
	{
		...
		ImGui::PushFont(c_menu::get().font);
		//PostProcessing::performFullscreenBlur(d, main_alpha);
		d->AddRectFilled(p, p + ImVec2(200, 21 + 20 * specs), ImColor(39, 39, 39, int(0 * main_alpha)));
		auto main_colf = ImColor(39, 39, 39, int(0 * main_alpha));
		auto main_coll = ImColor(39, 39, 39, int(0 * main_alpha));
		d->AddRectFilledMultiColor(p, p + ImVec2(100, 20), main_coll, main_colf, main_colf, main_coll);
		d->AddRectFilledMultiColor(p + ImVec2(100, 0), p + ImVec2(200, 20), main_colf, main_coll, main_coll, main_colf);
...
bool draw_config_button(const char* label, const char* label_id, bool load, bool save, int curr_config, bool create = false)
{
	...
	ImGui::PushStyleColor(ImGuiCol_PopupBg, ImVec4(29 / 255.f, 28 / 255.f, 49 / 255.f, csgo.gui.pop_anim * 0.85f)); //29, 28, 49
...
bool draw_water_button(const char* label, const char* label_id, bool load, bool save, int curr_config, bool create = false)
{
	...
	ImGui::PushStyleVar(ImGuiStyleVar_::ImGuiStyleVar_PopupBorderSize, 0);
	ImGui::PushStyleVar(ImGuiStyleVar_::ImGuiStyleVar_PopupRounding, 4);
	ImGui::PushStyleColor(ImGuiCol_PopupBg, ImVec4(38 / 255.f, 34 / 255.f, 43 / 255.f, csgo.gui.pop_anim * 0.85f));
	if (ImGui::BeginPopup(label_id, ImGuiWindowFlags_NoMove))
	{
...
#define VERSION crypt_str("leencheat")
void c_menu::waterwark()
{
	...
	std::string watermark = "leencheat";
	...
	ImGui::Begin("wwwwwwww", nullptr, ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoTitleBar | ImGuiWindowFlags_NoBackground | ImGuiWindowFlags_NoCollapse | ImGuiWindowFlags_NoScrollbar | ImGuiWindowFlags_::ImGuiWindowFlags_NoBringToFrontOnFocus | ImGuiWindowFlags_::ImGuiWindowFlags_NoBackground | ImGuiWindowFlags_::ImGuiWindowFlags_NoNavFocus | ImGuiWindowFlags_::ImGuiWindowFlags_NoNav);
	{
		...
		d->AddRectFilled(p, p + ImVec2(s.x, 21), ImColor(39, 39, 39));
		auto main_colf = ImColor(39, 39, 39);
		auto main_coll = ImColor(39, 39, 39);
...
void c_menu::initscene() {
	int x, y;
	ImVec2 sizemenu;
	sizemenu = ImGui::GetWindowSize();
	static int alpha = 0;
	...
	if (hooks::menu_open && alpha < 150 && alphatext < 255) {
	    ...
	    auto d = ImGui::GetWindowDrawList();
		auto main_colf = ImColor(39, 39, 39, int(0 * 1));
		ImColor rect_col = ImColor(0, 0, 0, alpha);
		ImColor image_col = ImColor(255, 255, 255, alphatext);
		d->AddRectFilled(ImVec2(0, 0), sizemenu, rect_col);
		IDirect3DTexture9* background = nullptr;
		ImGui::Image(background, sizemenu, ImVec2(0, 0), ImVec2(1, 1), ImVec4(1.f, 1.f, 1.f, 1.f));
		//ImGui::image(ImVec2(0, 0), sizemenu, ImColor(255, 255, 255), render::background);

	/*                            Inject Features                            */
		
			static int alphabg = 0;
			static int alphabgtext = 0;
			if (vars.misc.inject && alphabgtext < 255 && alphabg < 150)
			{
				alphabg += 3;
				alphabgtext += 5;
			}
			else if (!vars.misc.inject && alphabgtext > 0 && alphabg > 0)
			{
				alphabg -= 3;
				alphabgtext -= 5;
			}
			d->AddRectFilled(ImVec2(0, 0), sizemenu, rect_col);
			ImGui::Image(background, sizemenu, ImVec2(0, 0), ImVec2(1, 1), image_col);
...
void c_menu::draw(bool is_open)
{
		...
		uname = comp_name(); build = "[beta]"; build_at = __DATE__; build_at += " "; build_at += __TIME__; sponsor = "";
	...
	ImGui::Begin("##GUI", NULL, ImGuiWindowFlags_::ImGuiWindowFlags_NoScrollWithMouse | ImGuiWindowFlags_::ImGuiWindowFlags_NoDecoration | ImGuiWindowFlags_::ImGuiWindowFlags_NoBackground);
	{
		...
		{//draw
			...
			draw->AddRectFilled(ImVec2(p.x, p.y), ImVec2(p.x + s.x, p.y + s.y), ImColor(40, 39, 68, int(public_alpha * 255)));
			ImGui::GetForegroundDrawList()->AddRectFilled(ImVec2(p.x, p.y), ImVec2(p.x + s.x, p.y + s.y), ImColor(40, 39, 68, int(((preview_alpha) * 1.2f) * 255))); //33,33,33 // 32,29,36
			if (tab_anim_br > 0) {
			...
			ImGui::GetForegroundDrawList()->AddRectFilled(ImVec2(p.x, p.y), ImVec2(p.x + tab_anim, p.y + s.y - 20), ImColor(48, 46, 81, int(public_alpha * 255))); //39,39,39 // 38,34,43
			ImGui::PushFont(font);
			draw->AddText(ImVec2(p.x + 5, p.y + s.y - 20 + (10 - ImGui::CalcTextSize(std::string("leencheat " + build + " | build at: " + build_at + " | " + sponsor).c_str()).y / 2)), ImColor(220, 220, 220, int(public_alpha * 255)), std::string("leencheat " + build + " | build at: " + build_at + " | " + sponsor).c_str());
			...
			ImGui::PopFont();
			{
				...
				if (tab_anim > 220 && ss < 1.f)
					...
					ss -= 0.04f;
				ImGui::PushFont(name);
				ImGui::GetForegroundDrawList()->AddText(ImVec2(p.x + tab_anim / 2 - ImGui::CalcTextSize("leencheat").x / 2, p.y + 20 - ImGui::CalcTextSize("leencheat").y / 2), ImColor(vars.misc.menu_color.r() / 255.f, vars.misc.menu_color.g() / 255.f, vars.misc.menu_color.b() / 255.f, ImClamp(ss * 2.f, 0.f, 1.f)), "leencheat");
				ImGui::PopFont();
			}
		}
			...
			{//tabs
				{
					...
					if (ImGui::tab("L", "Legitbot", "Legitbot, Semirage", tab == 1, tab_anim) && last_tab == tab && !preview_reverse) tab = 1; if (ImGui::IsItemHovered())tab_hovered = true;
					...
					if (ImGui::tab("W", "Visuals", "Chams, ESP, World Visuals", tab == 3, tab_anim) && last_tab == tab && !preview_reverse) tab = 3; if (ImGui::IsItemHovered())tab_hovered = true;
					...
					ImGui::SetCursorPosY(295); // 345
					if (ImGui::tab("C", "Settings", "Configurations, Scripting", tab == 6, tab_anim) && last_tab == tab && !preview_reverse) tab = 6; if (ImGui::IsItemHovered())tab_hovered = true;
			...
			ImGui::PushFont(c_menu::get().font);
			{
				...
				case 0: //ragubot
				{
					...
					ImGui::BeginGroup(/*CHILDs SPACE START*/);
					{
						...
						else//main
						{
							...
							{
								ImGui::Checkbox(crypt_str("Hide Shots"), &vars.antiaim.hide_shots);
								ImGui::SameLine();
								draw_keybind(crypt_str(""), &vars.antiaim.hide_shots_key, crypt_str("##HOTKEY_HIDESHOTS"));


								ImGui::Checkbox(crypt_str("Doubletap"), &vars.ragebot.double_tap);
								ImGui::SameLine();
								draw_keybind(crypt_str(""), &vars.ragebot.double_tap_key, crypt_str("##HOTKEY_DOUBLETAP"));

								if (vars.ragebot.double_tap) {
									ImGui::SliderInt(crypt_str("DoubleTap Speed"), &vars.ragebot.double_tap_shift_value, 8, 18);
								}
							}
							ImGui::EndMenuChild();
				...
				case 1://legit
				{
					...
					ImGui::BeginGroup(/*CHILDs SPACE START*/);
					{
						...
						ImGui::MenuChild(crypt_str("Semirage Features  "), ImVec2(290, 140));
						{
				...
				case 2://antiaim
				{
					...
					ImGui::BeginGroup(/*CHILDs SPACE START*/);
					{
						...
						ImGui::MenuChild(vars.antiaim.antiaim_type ? crypt_str("Not available in Legit Mode ") : crypt_str("Direction"), ImVec2(290, 122));
						{
							if (!vars.antiaim.antiaim_type) {
					...
					ImGui::MenuChild("Changer", ImVec2(295 + 290, 420));
					{
						...
						if (current_profile == -1)
						{
							for (auto i = 0; i < vars.skins.skinChanger.size(); i++)
							{
								...
								ImGui::PushStyleColor(ImGuiCol_Button, ImVec4(63 / 255.f, 55 / 255.f, 74 / 255.f, 1.f)); //63, 55, 74
						...
						else
						{
							...
							ImGui::PushStyleColor(ImGuiCol_Button, ImVec4(63 / 255.f, 55 / 255.f, 74 / 255.f, 1.f));
							ImGui::PushStyleColor(ImGuiCol_ButtonActive, ImVec4(0, 0, 0, 0));
							ImGui::PushStyleColor(ImGuiCol_ButtonHovered, ImVec4(0, 0, 0, 0));
							ImGui::BeginGroup();
				...
				case 5://misc
				{
					...
					ImGui::BeginGroup(/*CHILDs SPACE START*/);
					{
						ImGui::MenuChild("Movement", ImVec2(290, 220));
						{
							...
							draw_combo(crypt_str("Auto strafe"), vars.misc.airstrafe, strafes, ARRAYSIZE(strafes));
						...
						ImGui::MenuChild("Logging", ImVec2(290, 196));
						{
							...
							ImGui::Checkbox(crypt_str("Hitsound"), &vars.esp.hitsound);
						}
						ImGui::EndMenuChild();
						...
						ImGui::MenuChild("Gameplay", ImVec2(290, 190));
						{
							...
							ImGui::Checkbox("Clantag", &vars.misc.clantag_spammer);
				...
				case 6://cefege pricupi eblan
				{
					...
					ImGui::BeginGroup(/*CHILDs SPACE START*/);
					{
						...
						ImGui::SetCursorPos(ImVec2(ImGui::GetCursorPosX() + 295, pad.y + 34));
						ImGui::MenuChild("Radio", ImVec2(290, 420));
						{
							//ImGui::Text("Soon");
							...
							ImGui::Combo(crypt_str("Radio"), &vars.misc.radiochannel, channels_radio, IM_ARRAYSIZE(channels_radio));

							ImGui::SliderFloat(("radio volume"), &vars.misc.radiovolume, 0.f, 100.f);
